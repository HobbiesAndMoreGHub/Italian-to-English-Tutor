...
function renderWordBank() {
    ...
    countEl.textContent = `${wordBank.length} word${wordBank.length !== 1 ? 's' : ''} saved`;
...
function editWordNote(index) {
    ...
    const newNote = prompt('Edit notes for "' + word.italian + '":', word.notes || '');
    if (newNote !== null) {
        word.notes = newNote;
        saveToLocalStorage();
        renderWordBank();
...
function handleFlashcardClick() {
    ...
    if (card.classList.contains('flipped')) {
        // Already showing answer — advance to next card
        nextCard();
    } else {
        flipCard();
    }
...
function generateAudioScript() {
    ...
    list.innerHTML = audioScript.map((item, i) => {
    ...
    document.getElementById('audio-script-count').textContent = `— ${audioScript.length} words, ${totalPlays} plays`;
    ...
    showToast(`Script generated: ${audioScript.length} words.`);
}
...
function speakNextWord() {
    if (!audioPlaying || audioIndex >= audioScript.length) {
        finishAudio();
        return;
    }

    const item = audioScript[audioIndex];
    ...
    const pauseDuration = parseFloat(document.getElementById('audio-pause').value) * 1000;
    ...
    document.getElementById('audio-now-word').textContent = `${item.italian}  →  ${item.english}${repeatLabel}`;
    document.getElementById('audio-progress-text').textContent = `${audioIndex + 1} / ${audioScript.length}`;
    document.getElementById('audio-progress-bar').style.width =
        `${((audioIndex + 1) / audioScript.length) * 100}%`;

    // Highlight current row
    document.querySelectorAll('.audio-script-row').forEach(r => r.classList.remove('playing'));
    const currentRow = document.querySelector(`.audio-script-row[data-index="${audioIndex}"]`);
    if (currentRow) {
    ...
    itUtterance.onend = () => {
        ...
        setTimeout(() => {
            ...
            enUtterance.rate = speed;

            enUtterance.onend = () => {
                if (!audioPlaying) return;
                audioRepeatCount++;
                if (audioRepeatCount < totalRepeats) {
                    // Repeat this word again (use half the between-words pause)
...
async function recordAudioScript() {
    ...
    showToast('A dialog will appear — select this tab and check "Share tab audio".');
